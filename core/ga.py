# core/ga.py — GA runner with live progress yielding
import numpy as np
...
def ga_run_live(
    ...
    """
    GA runner that yields progress updates every `yield_every` generations.

    Yields dicts with keys:
        generation      int   — current generation number
        best_fitness    float — best fitness so far (lower = better cost)
        fitness_history list  — cost per generation so far
        best_schedule   ndarray
        soc_list        list
        battery_action  list
        done            bool
    """
    T = len(demand)
    population = init_population(pop_size, T, max_solar, max_wind, max_hydro)
    ...
    for gen in range(int(generations)):
        ...
        fitness_history.append(-best_score)  # store as positive cost
        ...
        while len(new_pop) < pop_size:
            ...
            child = mutate(child, max_solar, max_wind, max_hydro, mutation_rate)
...
def ga_run(
    demand, max_solar, max_wind, max_hydro,
    battery_cap, init_soc, charge_rate, discharge_rate,
    pop_size, generations, mutation_rate, elitism_frac,
):
    """Blocking wrapper — runs to completion and returns final result."""
    result = None
